...
import com.example.backend.dto.response.file.ResFileDownloadDto;
import com.example.backend.dto.response.file.ResFileUploadDto;
...
public class FileService {
    ...
    public List<ResFileUploadDto> upload(Long boardId, List<MultipartFile> multipartFiles) throws IOException {
        ...
        List<File> fileEntitys = new ArrayList<>();
        for (MultipartFile multipartFile : multipartFiles) {
            ...
            String fileResourcePath = FOLDER_PATH + java.io.File.separator + filePath;

            // create folder if not created
            java.io.File f = new java.io.File(FOLDER_PATH);
            if (!f.exists()) {
                f.mkdir();
            }
    ...
    public ResFileDownloadDto download(Long fileId) throws IOException {
        ...
        byte[] content = Files.readAllBytes(new java.io.File(filePath).toPath());
    ...
    public void delete(Long fileId) {
        ...
        // local 파일을 삭제
        String filePath = FOLDER_PATH + java.io.File.separator + file.getFilePath();
