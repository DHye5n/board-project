...
public interface BoardRepository extends JpaRepository<Board, Long> {
    ...
    @Query(value = "SELECT b FROM Board b JOIN FETCH b.member",
                    countQuery = "SELECT COUNT(b) FROM Board b JOIN b.member")
    ...
    @Query(value = "SELECT b FROM Board b JOIN FETCH b.member WHERE b.title LIKE %:title%",
                    countQuery = "SELECT COUNT(b) FROM Board b JOIN b.member WHERE b.title LIKE %:title%")
    ...
    @Query(value = "SELECT b FROM Board b JOIN FETCH b.member WHERE b.content LIKE %:content%",
                    countQuery = "SELECT COUNT(b) FROM Board b JOIN b.member WHERE b.content LIKE %:content%")
    Page<Board> findAllContentContaining(String content, Pageable pageable);

    // 작성자 검색
    @Query(value = "SELECT b FROM Board b JOIN FETCH b.member WHERE b.member.name LIKE %:name%",
                    countQuery = "SELECT COUNT(b) FROM Board b JOIN b.member WHERE b.member.name LIKE %:name%")
    Page<Board> findAllNameContaining(String name, Pageable pageable);
}
